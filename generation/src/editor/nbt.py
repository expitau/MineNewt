...
def create_simple_nbt_class(tag_id, class_tag_name, tag_width, tag_parser):
    ...
    class DataNBTTag:
        ...
        def __repr__(self):
            return f'"[{type(self).clazz_name}] {self.tag_name}": "{str(self.tag_value)}"'
...
def create_string_nbt_class(tag_id):
    class DataNBTTag:
        ...
        def __repr__(self):
            return f'"[String] {self.tag_name}": "{self.tag_value}"'
...
def create_array_nbt_class(tag_id, tag_name, sub_type):
    class ArrayNBTTag:
        ...
        def __repr__(self):
            str_dat = ', '.join([str(c.get()) for c in self.children])
            return f'"[{type(self).clazz_name} {str(len(self.children))}] {self.tag_name}": [{str_dat}]'
...
def create_list_nbt_class(tag_id):
    class ListNBTTag:
        ...
        def __repr__(self):
            ...
            return f'"[List {str(len(self.children))} {str(self.sub_type_id)}] {self.tag_name}": {{{str_dat}}}'
...
def create_compund_nbt_class(tag_id):
    class CompundNBTTag:
        ...
        def __repr__(self):
            ...
            return f'"[Compound {str(len(self.children))}] {self.tag_name}": {{{str_dat}}}'
